...
def test_complete_task_marks_active_task_completed(tmp_path, monkeypatch):
    tasks_file = tmp_path / "tasks.json"
    history_file = tmp_path / "history.json"
    tasks_file.write_text("[]", encoding="utf-8")
    history_file.write_text("[]", encoding="utf-8")
    ...
    task = task_service.create_task(
        TaskCreate(
            title="买菜",
            scheduled_at=datetime(2026, 6, 3, 14, 0),
        )
    )
    ...
    assert task_service.task_store.find_by_id(task.id).completed is True
